...
void RunExtendedWorkflow(int16_t temperature, uint16_t humidity)
{
    workflow_sensor_report_t report = {};
    mic_audio_frame_t audioFrame    = {};
    secure_packet_t securePacket    = {};

    report.temperature_centi_c   = (temperature < 0) ? 0u : static_cast<uint32_t>(temperature);
    report.humidity_centi_percent = static_cast<uint32_t>(humidity);

    if (light_sensor_read(&report.light) != WORKFLOW_STATUS_OK)
    {
        ChipLogError(AppServer, "Light sensor read failed");
        return;
    }

    if (mic_audio_capture_frame(&audioFrame) != WORKFLOW_STATUS_OK)
    {
        ChipLogError(AppServer, "Microphone capture failed");
        return;
    }

    if (dsp_module_process_audio(&audioFrame, &report.audio_features) != WORKFLOW_STATUS_OK)
    {
        ChipLogError(AppServer, "DSP processing failed");
        return;
    }

    if (ml_model_classify(&report.audio_features, &report.classification) != WORKFLOW_STATUS_OK)
    {
        ChipLogError(AppServer, "ML classification failed");
        return;
    }

    if (security_module_wrap_report(&report, &securePacket) != WORKFLOW_STATUS_OK)
    {
        ChipLogError(AppServer, "Security packet wrapping failed");
        return;
    }

    if (network_manager_publish_local(&securePacket) != WORKFLOW_STATUS_OK)
    {
        ChipLogError(AppServer, "Ad hoc network publish failed");
        return;
    }

    if (aws_cloud_publish_report(&securePacket) != WORKFLOW_STATUS_OK)
    {
        ChipLogError(AppServer, "AWS cloud publish failed");
        return;
    }

    ChipLogDetail(AppServer,
                  "Workflow report: lux=%lu rms=%lu bird=%s confidence=%u%% packets=%lu cloud=%lu",
                  static_cast<unsigned long>(report.light.lux),
                  static_cast<unsigned long>(report.audio_features.rms_level),
                  report.classification.species,
                  static_cast<unsigned int>(report.classification.confidence_percent),
                  static_cast<unsigned long>(network_manager_get_forward_count()),
                  static_cast<unsigned long>(aws_cloud_get_publish_count()));
}
...
void SensorActionTriggered(chip::System::Layer * aLayer, void * aAppState)
{
    ...
    RunExtendedWorkflow(temperature, humidity);
}
...
CHIP_ERROR Init()
{
    ...
    VerifyOrDieWithMsg(light_sensor_init() == WORKFLOW_STATUS_OK, AppServer, "Failed to initialize light sensor module!");
    VerifyOrDieWithMsg(mic_audio_init() == WORKFLOW_STATUS_OK, AppServer, "Failed to initialize mic audio module!");
    VerifyOrDieWithMsg(dsp_module_init() == WORKFLOW_STATUS_OK, AppServer, "Failed to initialize DSP module!");
    VerifyOrDieWithMsg(ml_model_init() == WORKFLOW_STATUS_OK, AppServer, "Failed to initialize ML module!");
    VerifyOrDieWithMsg(security_module_init() == WORKFLOW_STATUS_OK, AppServer, "Failed to initialize security module!");
    VerifyOrDieWithMsg(network_manager_init() == WORKFLOW_STATUS_OK, AppServer, "Failed to initialize network module!");
    VerifyOrDieWithMsg(aws_cloud_init() == WORKFLOW_STATUS_OK, AppServer, "Failed to initialize AWS cloud module!");
